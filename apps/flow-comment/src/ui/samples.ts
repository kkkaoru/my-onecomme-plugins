...
const SUPER_CHAT_TIERS: readonly SuperChatTier[] = [
  { amount: '¥100', background: 'rgb(30, 136, 229)', label: '青' },
  { amount: '¥200', background: 'rgb(0, 184, 212)', label: '水色' },
  { amount: '¥500', background: 'rgb(0, 191, 165)', label: '緑' },
  { amount: '¥1,000', background: 'rgb(255, 202, 40)', label: '黄' },
  { amount: '¥2,000', background: 'rgb(245, 124, 0)', label: '橙' },
  { amount: '¥5,000', background: 'rgb(194, 24, 91)', label: '桃' },
  { amount: '¥10,000', background: 'rgb(208, 0, 0)', label: '赤' },
]
...
const superChatSample = (tier: SuperChatTier, index: number): PreviewSample => {
  ...
  return {
    ...
    label: `スパチャ ${tier.amount}（${tier.label}）`,
  }
}

const fixedSample = (comment: FlowComment, label: string): PreviewSample => ({ comment, label })
